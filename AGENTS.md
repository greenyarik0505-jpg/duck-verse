...
### Крок 3: Smart Commit, пуш робочої гілки та Pull Request
- **Прямий push у `main` заборонений.** `main` — стабільна гілка, яку змінюють тільки через Pull Request.
- Кожна зміна повинна бути прив'язана до задачі Jira через ключ `SCRUM-XX`.
- Після перевірки створити коміт у форматі:
  ```
  feat(SCRUM-XX): короткий опис нової функції
  fix(SCRUM-XX): короткий опис виправлення
  refactor(SCRUM-XX): короткий опис рефакторингу
  docs(SCRUM-XX): короткий опис документації
  ```
- Відправляти потрібно тільки робочу гілку:
  ```powershell
  git push -u origin <назва-робочої-гілки>
  ```
- Після push обов'язково відкрити Pull Request у `main`:
  1. Натиснути **Compare & pull request**.
  2. Перевірити напрямок: робоча гілка → `main`.
  3. У назві вказати ключ Jira, наприклад `[SCRUM-18] Flappy Duck`.
  4. Заповнити шаблон Pull Request.
  5. Додати всіх потрібних рев'юверів у правій панелі **Reviewers**.
  6. Перевести задачу Jira у **In Review**.
- PR не можна зливати, поки не пройшли CI-перевірки, не вирішені всі коментарі та не отримані необхідні схвалення.
- Merge у `main` виконує Team Lead або призначений ментор. Після merge гілку потрібно видалити, а задачу Jira перевести у **Done**.
...
## 🤝 5A. Обов’язковий командний GitHub workflow

> **Золоте правило:** `main` не редагуємо напряму. Робота завжди проходить шляхом:
> **Jira-задача → робоча гілка → коміти → Pull Request → рев'ю → CI → merge → Done.**

### Перед початком роботи
1. Відкрити призначену задачу в Jira.
2. Перевести її у **In Progress / В работе**.
3. Синхронізувати локальний `main`:
   ```powershell
   git switch main
...
### Назва робочої гілки
Гілка створюється **для кожної задачі окремо**:

```powershell
git switch -c feature/SCRUM-18-flappy-duck
```
...
Дозволені типи:
- `feature/` — нова функція або гра;
- `fix/` — виправлення помилки;
- `refactor/` — зміна структури без зміни поведінки;
- `docs/` — документація.

### Правила комітів
- Один коміт описує одну логічну зміну.
- Кожен коміт містить Jira-ключ.
- Перед commit обов'язково виконати:
  ```powershell
  npm run lint
  npm run build
  ```
- Якщо lint або build падає, роботу спочатку виправити, а вже потім робити push.

### Правила Pull Request
- Base branch завжди `main`.
- У назві PR обов'язково є Jira-ключ: `[SCRUM-XX] Короткий опис`.
- Опис PR заповнюється за шаблоном і містить: що зроблено, як перевірено, посилання на Jira та Vercel Preview.
- Усі зміни проходять через PR; навіть невелика правка документації не пушиться напряму в `main`.
- Автор PR сам перевіряє вкладку **Checks** і відповідає на кожен коментар рев'ю.
- Після нових виправлень автор повторно запитує рев'ю, якщо GitHub його не запросив автоматично.

### Рев'ювери
Для командної задачі в PR потрібно зазначити всіх учасників, які мають перевірити результат:
- **Yarik0505 / Team Lead**;
- **Степаненко Дмитро**;
- **Кирил Пушкарук**;
- додатково — ментор або профільний рев'ювер, якщо його призначено в Jira.

У GitHub це робиться через **Reviewers → Request review**. Рев'ювер не просто натискає Approve: він перевіряє код, запускає або переглядає CI, залишає зрозумілі коментарі та підтверджує, що зміна відповідає задачі Jira.

### Що дозволено під час рев'ю
- Виправлення робляться у тій самій робочій гілці.
- Нові commits автоматично оновлюють існуючий PR.
- Заборонено приховувати коментарі, видаляти зауваження або створювати новий PR замість виправлення поточного.
- Якщо є **Request changes**, спочатку виправити зауваження, потім знову запросити рев'ю.
- Merge виконується тільки після схвалення та успішних перевірок.

### Налаштування, які власник репозиторію має застосувати до `main`
У **Settings → Branches / Rulesets** потрібно ввімкнути:
- Require a pull request before merging;
- мінімум **2 approving reviews**;
- Require conversation resolution;
- Require status checks to pass;
- Dismiss stale approvals when new commits are pushed;
- заборонити force push і видалення `main`;
- не дозволяти обходити правила захисту.

Для автоматичного призначення рев'юверів можна додати `.github/CODEOWNERS`, але в ньому потрібно вказати справжні GitHub usernames учасників. Після створення PR автор все одно перевіряє, що всі потрібні люди відображаються в **Reviewers**.

### Що заборонено
- працювати без задачі Jira;
- робити зміни без робочої гілки;
- пушити напряму в `main`;
- відкривати PR без Jira-ключа;
- зливати PR із червоним CI;
- зливати PR без рев'ю;
- використовувати `git push --force` у спільних гілках;
- позначати задачу **Done**, доки PR не злитий у `main`.
