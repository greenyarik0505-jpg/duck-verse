...
### 📋 Детальний статус та завдання команди за етапами

#### 🟢 Етап 1: Базовий фундамент (Завершено ✅)
*Кожен заклав основу своєї частини проекту:*
* **Yarik0505** ➡️ [`SCRUM-14`](https://gta6-sliv-cyberleek.atlassian.net/browse/SCRUM-14): **Next.js 15, Tailwind CSS та Vercel CI/CD** `[ГОТОВО ✅]`
  *(Розгорнуто каркас сайту на Vercel, налаштовано Next.js 15 App Router, Tailwind CSS та автоматичний GitHub Actions пайплайн).*
* **Степаненко Дмитро** ➡️ [`SCRUM-7`](https://gta6-sliv-cyberleek.atlassian.net/browse/SCRUM-7): **GD: Базова фізика куба та керування** `[ГОТОВО ✅]`
  *(Створено фізику куба: постійний біг вперед, гравітація, стрибок по Space/тапу та поворот на 90° у повітрі).*
* **Кирил Пушкарук** ➡️ [`SCRUM-9`](https://gta6-sliv-cyberleek.atlassian.net/browse/SCRUM-9): **GD: Музичний рушій та ритм (130 BPM)** `[ГОТОВО ✅]`
  *(Написано процедурний аудіо-синтезатор на 130 BPM через Web Audio API та звукові ефекти стрибка/вибуху).*

#### 🟡 Етап 2: Геймплей та інтеграція перешкод (В активній роботі ⚡)
*Куб уже бігає, музика грає — наповнюємо гру геймплеєм:*
* **Степаненко Дмитро** ➡️ [`SCRUM-8`](https://gta6-sliv-cyberleek.atlassian.net/browse/SCRUM-8): **GD: Перешкоди, інтерактивні об'єкти та хітбокси** `[В РОБОТІ ⚡]`
  *(Додає шипи з чесними хітбоксами, блоки-платформи, жовті батути для підкиду та жовті сфери для стрибків у повітрі).*
* **Кирил Пушкарук** ➡️ [`SCRUM-10`](https://gta6-sliv-cyberleek.atlassian.net/browse/SCRUM-10): **GD: Дизайн першого рівня (Neon Madness) та прогрес-бар** `[В РОБОТІ ⚡]`
  *(Розставляє перешкоди під 130 BPM біт музики від 0% до 100%, додає прогрес-бар та лічильник спроб).*
* **Yarik0505** ➡️ [`SCRUM-15`](https://gta6-sliv-cyberleek.atlassian.net/browse/SCRUM-15): **Next.js: Архітектура Game Hub та Game Registry** `[В РОБОТІ ⚡]`
  *(Розробляє вітрину хабу, модульний реєстр `lib/games/registry.js`, фільтрацію за категоріями та модальний лаунчер із захистом від витоків пам'яті).*

#### 🟣 Етап 3: Візуальні ефекти, бекенд та магазин (Стартує ⚡)
*Доводимо гру до ідеального вигляду та додаємо збереження прогресу:*
* **Степаненко Дмитро** ➡️ [`SCRUM-11`](https://gta6-sliv-cyberleek.atlassian.net/browse/SCRUM-11): **GD: Візуальні ефекти, неоновий шлейф і частинки** `[К ВИКОНАННЮ ⏳]`
  *(Неоновий шлейф за кубом, ефект вибуху на 20+ осколків при смерті та світіння об'єктів).*
* **Кирил Пушкарук** ➡️ [`SCRUM-16`](https://gta6-sliv-cyberleek.atlassian.net/browse/SCRUM-16): **Backend: Збереження прогресу, рекорди та лідерборд у Vercel** `[К ВИКОНАННЮ ⏳]`
  *(Next.js API Route для збереження відсотків проходження, спроб та зароблених монет у Vercel).*
* **Yarik0505** ➡️ [`SCRUM-12`](https://gta6-sliv-cyberleek.atlassian.net/browse/SCRUM-12): **GD: Магазин кастомізації та скіни куба** `[В РОБОТІ ⚡]`
  *(Система збереження скінів у LocalStorage, вітрина магазину, динамічне 60 FPS прев'ю кубика та синхронізація кольорів).*

#### 🔴 Етап 4: Фінал і реліз (Спільний 🚀)
* **Yarik0505 (і вся команда)** ➡️ [`SCRUM-13`](https://gta6-sliv-cyberleek.atlassian.net/browse/SCRUM-13): **GD: Тестування Input Lag (60 FPS) та фінальний реліз** `[В РОБОТІ ⚡]`
  * Тестування гри на ПК і смартфонах через Vercel Preview.
  * Усунення затримок вводу (Input Lag < 16 мс) та стабілізація 60 FPS через Fixed Delta Time.
  * Мердж усіх фіч у гілку `main` та закриття спринту в Jira!
